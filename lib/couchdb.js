...
Db.prototype.saveAttachment = function(file, docId, options, cb) {
  ...
  fs.readFile(file, 'binary', function (er, data) {
    ...
    self.request({
      ...
      path: '/'+querystring.escape(docId)+'/'+options.name+(options.rev ? '?rev='+options.rev : ''),
      headers: {
...
Db.prototype.removeAttachment = function(docId, attachmentId, docRev, cb) {
  return this.request({
    method: 'DELETE',
    path: '/'+querystring.escape(docId)+'/'+attachmentId,
    query: {rev: docRev}
  }, cb)
};
...
Db.prototype.getAttachment = function(docId, attachmentId, cb) {
  return this.request({
    path: '/'+querystring.escape(docId)+'/'+querystring.escape(attachmentId),
